...
// PRISMTests
//
// The §5.18 per-app rule logic: the resolver (including the two-clients-at-
...
final class AppRuleTests: XCTestCase {
    ...
    private func settings(_ rules: [AppRule],
                          defaultAccess: AppAccess = .allow) -> AppRulesSettings {
        var settings = AppRulesSettings()
    ...
    // MARK: - Defaults (§5.18 ships off)

    func testDefaultsAreInert() {
        let settings = AppRulesSettings()
    ...
    func testDisabledSettingsAllowAndStyleNothing() {
        ...
        XCTAssertEqual(settings.access(for: zoom), .allow)
        XCTAssertEqual(settings.access(for: "com.example.anything"), .allow)
    ...
    func testUnnamedAppTakesTheDefault() {
        ...
        XCTAssertEqual(allowing.access(for: facetime), .allow)

        let blocking = settings([AppRule(signingID: zoom, access: .allow)],
                                defaultAccess: .block)
        XCTAssertEqual(blocking.access(for: facetime), .block)
        XCTAssertEqual(blocking.access(for: zoom), .allow)
    }

    func testMatchingIsWholeStringNotPrefix() {
        // The whole point of matching on a signing ID is that it is an
        // identity; a look-alike must not inherit the rule. Case is the one
        // thing folded — signing identifiers are case-insensitive in practice
        // and users type them into the editor by hand.
        let settings = settings([AppRule(signingID: zoom, access: .block)])
        XCTAssertEqual(settings.access(for: zoom), .block)
        XCTAssertEqual(settings.access(for: "us.zoom.xos.helper"), .allow)
        XCTAssertEqual(settings.access(for: "US.ZOOM.XOS"), .block)
        XCTAssertEqual(settings.access(for: "us.zoom"), .allow)
    }
    ...
    func testPRISMItselfIsNeverBlocked() {
        ...
        XCTAssertEqual(
            settings.access(for: AppRuleResolver.selfSigningID), .allow)
        XCTAssertFalse(
    ...
    func testFirstRuleForAnAppWins() {
        let settings = settings([
            AppRule(signingID: zoom, presetID: meeting, access: .allow),
            AppRule(signingID: zoom, presetID: studio, access: .block),
        ])
        XCTAssertEqual(settings.access(for: zoom), .allow)
    ...
    func testEmptySigningIDMatchesNothing() {
        let settings = settings([
            AppRule(signingID: "   ", presetID: studio, access: .block),
        ], defaultAccess: .allow)
        XCTAssertEqual(settings.access(for: ""), .allow)
    ...
    func testRuleWithoutAPresetDecidesAccessOnly() {
        let settings = settings([AppRule(signingID: zoom, presetID: nil, access: .allow)])
        XCTAssertEqual(settings.access(for: zoom), .allow)
    ...
    func testBlockedAppNeverDrivesTheLook() {
        ...
        let settings = settings([AppRule(signingID: zoom, presetID: studio, access: .block)])
        XCTAssertNil(AppRuleResolver.presetMatch(clients: [zoom], in: settings))
    }
    ...
    func testHigherRuleWithoutAPresetDoesNotVetoALowerOne() {
        ...
        let settings = settings([
            AppRule(signingID: zoom, presetID: nil, access: .allow),
            AppRule(signingID: facetime, presetID: studio),
        ])
    ...
    func testBlockedHigherRuleDoesNotVetoALowerOne() {
        let settings = settings([
            AppRule(signingID: zoom, presetID: meeting, access: .block),
            AppRule(signingID: facetime, presetID: studio),
        ])
        XCTAssertEqual(AppRuleResolver.presetMatch(clients: [zoom, facetime], in: settings),
                       AppRuleMatch(signingID: facetime, presetID: studio))
    ...
    private func policyJSON(_ settings: AppRulesSettings) throws -> [String: Any] {
    ...
    func testPolicyPayloadKeysMatchTheExtensionMirror() throws {
        ...
        XCTAssertEqual(object["allowed"] as? [String], [facetime.lowercased()])
    ...
    func testFlatteningTakesTheFirstRulePerApp() {
        ...
        XCTAssertEqual(policy.allowed, [facetime.lowercased()])
    ...
    private func decodeSettings(_ json: String) throws -> AppRulesSettings {
        try JSONDecoder().decode(AppRulesSettings.self, from: Data(json.utf8))
    }

    func testEmptyObjectDecodesToDefaults() throws {
        XCTAssertEqual(try decodeSettings("{}"), AppRulesSettings())
    }
    ...
    func testUnknownEnumValueFallsBackToAllow() throws {
        ...
        XCTAssertEqual(decoded.defaultAccess, .allow)
        XCTAssertEqual(decoded.rules[0].access, .allow)
        XCTAssertEqual(decoded.access(for: zoom), .allow)
    }
    ...
    func testRoundTripPreservesEverything() throws {
        ...
        XCTAssertEqual(try JSONDecoder().decode(AppRulesSettings.self, from: data), original)
    }
    ...
    func testCameraClientDerivesItsDisplayName() {
        XCTAssertEqual(CameraClient(signingID: zoom).displayName, "Zoom")
