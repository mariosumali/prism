...
// PRISM
//
// Microphone cleanup on the capture path (SPEC §5.17): one picker over a
...
// MARK: - DSP program
//
// `VoiceCleanupMode` and `VoiceCleanupSettings` live in
// PRISM/Pipeline/Settings/VoiceCleanupSettings.swift with every other
// persisted settings struct. This file owns only what the modes mean in
// samples.
...
public final class VoiceCleanup {
    ...
    /// The mode table (§5.17). Studio is not "Clean up, more": it expands
    ...
    /// `amount` scales only the *depth* of the noise removal, and it is the
    /// dB depth it scales, not the linear gain: half of "20 dB down" is
    /// 10 dB down, which is what a listener would call half. It deliberately
    /// does not touch the compressor or the EQ — the mode picks what kind of
    /// tidying happens, the amount picks how hard the room is pushed under,
    /// and mixing the two into one number would make the slider mean
    /// something different in each mode.
    ///
    /// Defaults to 1 so the mode table can be read and tested at full depth.
    static func program(for mode: VoiceCleanupMode,
                        amount: Double = 1) -> CleanupProgram {
        ...
        let depth = Float(min(max(amount, 0), 1))
        p.lowOpenRatio = linear(db: studio ? 14 : 11)
        p.lowFloorGain = powf(studio ? 0.03 : 0.10, depth)
        p.highOpenRatio = linear(db: studio ? 16 : 12)
        p.highFloorGain = powf(studio ? 0.02 : 0.08, depth)
