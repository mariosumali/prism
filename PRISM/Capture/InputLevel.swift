...
// answer until a mic check was running (SPEC §5.17). Two pieces, both
...
final class MicWatch {
    ...
    /// Adopts the user's persisted watch settings (§5.17). Only the three
    /// the settings actually expose move: the gap and clear windows are
    /// properties of how speech is shaped, not preferences, and there is no
    /// question a user could sensibly be asked about them (§8.7).
    ///
    /// Thresholds are stored in dBFS because that is the unit a level is
    /// discussed in, and converted here — once, on the main thread — because
    /// the comparison on the hot path must stay a compare against a linear
    /// RMS.
    func apply(_ settings: MicWatchSettings) {
        tuning.speechRMS = pow(10, settings.clampedThresholdDB / 20)
        tuning.sustainSeconds = settings.clampedSustainSeconds
        tuning.holdoffSeconds = settings.clampedReminderIntervalSeconds
    }
