...
public final class OverlayStage: EffectStage {
    ...
    private func encodeLayer(_ layer: OverlayLayer,
        ...
        params.uvTransform = Self.placement(
            layer: layer,
            contentSize: CGSize(width: texture.width, height: texture.height),
            outputSize: CGSize(width: destination.width, height: destination.height))
        ...
        params.opacity = Float(min(max(layer.opacity, 0), 1))
    ...
    static func placement(layer: OverlayLayer,
        ...
        let centerX = 0.5 + Float(min(max(layer.offsetX, -1), 1)) * 0.5
        let centerY = 0.5 + Float(min(max(layer.offsetY, -1), 1)) * 0.5
        let radians = Float(min(max(layer.rotationDegrees, -180), 180)) * .pi / 180

        var m = translationMatrix(-centerX, -centerY)
