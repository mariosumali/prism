...
// PRISM
//
// The main window's Apps pane (§5.15): the ordered rule list, the default
...
struct AppsPane: View {
        ...
        Section("Per-app rules") {
            ...
            if !state.clientsInUse.isEmpty {
                LabeledContent("Using PRISM Camera now",
                               value: state.clientsInUse.map(\.name).joined(separator: ", "))
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: Metrics.itemGap) {
            HStack(spacing: Metrics.itemGap) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(CMIOSink.displayName(forSigningID: rule.signingID))
                        .font(.body)
                    Text(rule.signingID)
                        .font(.caption2)
                        .foregroundStyle(.secondary)
        ...
        Section("Add a rule") {
            ForEach(unruledClients) { client in
                Button {
                    ...
                    Label("Add rule for \(client.name)", systemImage: "plus")
                }
            }
            HStack(spacing: Metrics.itemGap) {
    ...
    private var unruledClients: [CameraClient] {
        state.clientsInUse.filter { client in
            !state.appRules.rules.contains { $0.signingID == client.signingID }
        }
    }
    ...
    private func confirmBlockingLiveApp(_ rule: AppRule) {
        let name = CMIOSink.displayName(forSigningID: rule.signingID)
        let alert = NSAlert()
