...
public final class AppState: ObservableObject {
    ...
    @Published public var clientsInUse: [CameraClient] = []
    /// §5.15 — apps the extension is refusing right now. Non-empty only
    ...
    /// §5.15 per-app rules. Behaviour rather than look, like StudioSettings:
    /// a preset that carried rules could apply itself.
    ...
    /// Dirty flag for the §5.15 'polc' write, same shape as the one above.
    ...
    private enum DefaultsKey {
        ...
        static let appRules = "PRISM.appRules"
    }
    ...
    private func wireSink() {
        cmioSink.onClientsChanged = { [weak self] clients in
            guard let self else { return }
            self.clientsInUse = clients
            self.updateMenuBarState()
            self.reconcileCaptures()       // a first client starts capture
            self.reconcileAppRules()       // §5.15: who is watching decides the look
        }
    ...
    private func pollTick() {
        ...
        // §5.15: same dirty-flag treatment for 'polc', and it matters more —
    ...
    public func updateEditing(_ mutate: (inout PipelineConfiguration) -> Void) {
        if draftConfig != nil {
            ...
            // Editing the look by hand takes the wheel back from a §5.15
    ...
    public func requestPublishedFormatsChange(_ formats: [VideoFormat]) {
        ...
        let apply: () -> Void = { [weak self] in
            ...
            if let first = self.clientsInUse.first {
                self.postNotification(
                    body: "Reselect PRISM Camera in \(first.name) to use the new format")
            }
        ...
        let names = clientsInUse.map(\.name).joined(separator: " and ")
        let alert = NSAlert()
    ...
    public func selectPreset(_ id: UUID) {
        ...
        // Choosing a preset by hand takes the wheel back from a §5.15 rule:
    ...
    public func saveCurrentAsPreset(named name: String) {
        ...
        // §5.15 rule-driven look is somebody else's preset already, and
    ...
    // MARK: - Intents: per-app rules (§5.15)
    ...
    private func reconcileAppRules() {
        let match = AppRuleResolver.presetMatch(
            clients: clientsInUse.map(\.signingID), in: appRules)
        ...
        if appRules.announcesPresetChanges {
            ...
            postNotification(
                body: "\(CMIOSink.displayName(forSigningID: match.signingID)) connected — \(preset.name) preset applied")
        }
    }
    ...
    private func updateBlockedWarning() {
        if let first = blockedClients.first {
            warning = WarningMessage(
                text: "PRISM is blocking \(first.name) from using PRISM Camera",
                action: .clearBlocks)
    ...
    public func addAppRule(signingID: String,
        ...
        if let index = appRules.rules.firstIndex(where: { $0.signingID == trimmed }) {
            appRules.rules[index].access = access
            appRules.rules[index].presetID = presetID
            return
        }
        appRules.rules.append(
            AppRule(signingID: trimmed, access: access, presetID: presetID))
    }
    ...
    public func isStreamingNow(_ signingID: String) -> Bool {
        clientsInUse.contains { $0.signingID == signingID }
    }
    ...
    private func loadPersistedState() {
        ...
        if let data = UserDefaults.standard.data(forKey: DefaultsKey.appRules),
           let decoded = try? JSONDecoder().decode(AppRuleSettings.self, from: data) {
            appRules = decoded
        }
    ...
    /// A §5.15 rule-driven look is no more the user's saved configuration
    ...
    private func persistAppRules() {
        if let data = try? JSONEncoder().encode(appRules) {
            UserDefaults.standard.set(data, forKey: DefaultsKey.appRules)
        }
    }
