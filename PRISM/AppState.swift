...
public final class AppState: ObservableObject {
    ...
    /// §5.15 — the user's global chords. Written through setShortcut and
    ...
    /// §5.16 — whether App Intents may drive PRISM. Off until asked for:
    ...
    /// §5.17 — this session's history, in memory only.
    ...
    /// How many shortcut recorders are armed (§5.15); the tap is off while
    /// any of them is.
    ...
    /// The running instance, for App Intents (§5.16) — which are constructed
    ...
    private func wireLatencyMonitor() {
        ...
        monitor.onAutoDisable = { [weak self] id in
            ...
            // §5.17: the warning row is transient and the next event replaces
    ...
    /// §5.17. The device lists arrive whole, so the story ("the Logitech
    ...
    private func wireSink() {
        cmioSink.onClientsChanged = { [weak self] names in
            guard let self else { return }
            for name in names where !self.clientsInUse.contains(name) {
                self.sessionLog.record(.clients, "\(name) started using PRISM Camera")
            }
            for name in self.clientsInUse where !names.contains(name) {
                self.sessionLog.record(.clients, "\(name) stopped using PRISM Camera")
            }
    ...
    /// The one place a bindable action turns into behaviour, so the tap, the
    /// Shortcuts intents and anything added later cannot drift apart.
    ...
    // MARK: - Intents: control surface (§5.15, §5.16)

    /// §5.15. Said the same way wherever a chord is refused, because it is
    ...
    private func updateMenuBarState() {
        ...
        if newState != menuBarState {
            // §5.17: the glyph already summarises everything that changes
